...
| D20 | 2026-09-01 | **Evermore is the `healthy_catering` product, and the build is fully fresh.** Steven supplied the design artifact (22 screens across 4 artboards, branded PT Evermore Nutrisi Indonesia / evermore.co.id / EVM- order codes) and confirmed the promoted docs are the spec. Code is written from scratch rather than ported: `internal/platform/*` too, which **deviates from CLAUDE.md §2's "carry it over rather than reinvent"** — recorded as a deliberate deviation, not an oversight. | Steven's explicit choice when asked, 2026-09-01. The trade is slower delivery for code written against this artifact rather than inherited. | CLAUDE.md §1 §2, PROGRESS |
| D21 | 2026-09-01 | **The new build coexists with the running deployment rather than replacing it.** `evermore.service` (healthy_catering) keeps :8090, `/etc/evermore/evermore.env`, MinIO bucket `evermore` and the evermore.co.id server names. This build is `evermore-app.service` on :8091, API 127.0.0.1:8082, config `/etc/evermore/app.env`, database `evermore` + `evermore_test`, MinIO bucket `evermore-app`, Redis db 1. | Steven chose coexistence over takeover. Stopping a running deployment to claim a port is not reversible in one step; a second port is. Cut-over is his call, later. | 14, PROGRESS, deploy/ |
| D22 | 2026-09-01 | **Satellite credentials carried from healthy_catering; signing keys generated fresh.** MinIO access/secret, both Google Maps keys and the WAHA API key are reused per Steven's instruction. `JWT_SIGNING_KEY` and `TOTP_ENCRYPTION_KEY` are newly generated. | Steven said credentials may be taken from that project, and for shared infrastructure that is correct. A signing key is not shared infrastructure — it is a security boundary, and reusing one would make a token minted for one system valid in the other. | app.env, 12 |
| D23 | 2026-09-01 | **A menu card shows the price for ONE portion, not the cheapest tier.** The listing resolves the tier containing qty 1 (Rp 78.000), not `min()` across tiers (Rp 71.000, the 10+ band). | Found by looking at the rendered page: a card showed Rp 71.000 beside a button that charges Rp 78.000 for a single portion. The artifact shows Rp 78.000 on menu cards for this reason. | adapter/postgres, 02 |
| D24 | 2026-09-01 | **An admin may take any order transition staff can.** The §4.1 machine lists customer/staff on the cancel edges; admin is granted those implicitly rather than being enumerated on each. `REFUNDED` stays admin-only. | Enumerating admin on every staff edge makes it easy to omit one, and an omitted one reads as the administrator being locked out of their own system. Caught by the transition-table test. | domain/order, 02 |
| D25 | 2026-09-01 | **`upgrade-insecure-requests` and HSTS are emitted only in production.** On a plain-HTTP host the browser rewrites every subresource to https:// and the page loses all its assets. | Found by probing the deployed URL rather than localhost, where browsers treat the origin as trustworthy and the directive is inert. The headers are correct and withheld until TLS terminates. | adapter/http, 12, RUN-WHEN-BACK |
| D26 | 2026-09-01 | **The transactional surface is server-rendered Go templates, not a React SPA.** D13 reserved React 18 + Vite for the signed-in screens; cart, checkout, payment, orders, credits and the whole back office are instead `html/template` like the public site. | Three reasons: "as same as possible" to the artifact is reached faster this way, the screens work with JavaScript disabled, and the approach is already proven in this repo. **Recorded as a deviation for Steven to rule on** — it is reversible, because the JSON API a SPA would consume exists either way. | CLAUDE.md §3, 02, PROGRESS |
| D27 | 2026-09-01 | **Reservation walks the ranked kitchen list under lock, rather than committing to the router's first choice.** `routing.Rank` returns every eligible kitchen; checkout tries each in order and falls through when one turns out to be full. | Free capacity is read outside the row lock. Committing to one kitchen meant 12 concurrent checkouts against 6 available portions sold only 4, telling four customers there was no capacity while another kitchen had room. Found by tightening the oversell test until the limit actually bound. | domain/routing, app/ordering, 01 §5.3 |
| D28 | 2026-09-01 | **The seed resolves every cross-reference by natural key, never by the id it wanted to write.** Nineteen sites changed. | Inserts are ON CONFLICT (natural key) DO NOTHING, so a row already present under a different id meant the seed's own id was never written and every reference to it broke with a foreign-key violation. Idempotency that only holds on an empty database is not idempotency. | app/seed |
| D29 | 2026-09-01 | **A meal with an incomplete nutrition panel cannot be published.** The back-office publish action refuses it and the disabled button states the reason. | §5.2b says a meal missing a component's panel is incomplete rather than summed; publishing one would under-report the panel on the customer screen. A disabled control explains itself (CLAUDE.md §7). | adapter/http, 01 §5.2b |
| D30 | 2026-09-01 | **The beige sheet is retired as a page ground; the deep green `#1C3D34` is the ground everywhere, with `#234A40` as an alternating band.** Applied on `<main>` so every page inherits it. | Steven, 2026-09-01: the beige read as yellow and he asked for the homepage green across the site. The band exists because a single flat ground lost the page's rhythm. `#2A5749` was the better-looking band and is **rejected**: muted `#CCBDAA` lands at 4.47 on it, under the 4.5 body floor — recorded in design.md §3 so it is not re-proposed. | 10, design.md §3, tokens.css |
| D31 | 2026-09-01 | **`scripts/contrast.py` now verifies design.md rather than claiming to.** It compares its computed ratios against its recorded table, then asserts every recorded ratio actually appears in the sheet. | Its closing line used to read "match the numbers recorded in design.md §3" while comparing only against its own hardcoded table — the incident log's last row is a guard that silently stopped guarding. On its first run the new check found two ratios (3.90, 6.89) that lived only in D5 and had never reached the sheet. | scripts/contrast.py, design.md §3 |
